...
const sections = [
  ...
  },
  {
    ...
    items: [
      ...
      "Geração de 3 rotas candidatas com score híbrido e leitura prática.",
      "Tela de detalhe mostrando justificativa, score e pontos de atenção.",
      "Histórico de rotas salvas como continuidade do produto.",
    ],
  },
  {
    ...
    <AppShell>
      <main className="grid gap-6">
        <section className="rounded-[32px] border border-emerald-100 bg-white p-6 shadow-[0_16px_50px_rgba(16,24,40,0.06)] md:p-8">
          <div className="text-xs font-semibold uppercase tracking-[0.22em] text-emerald-600">Manual</div>
          <h1 className="mt-3 text-4xl font-semibold tracking-[-0.06em] text-emerald-950">Como operar o MVP do Ironfit</h1>
        ...
        <section className="grid gap-4 xl:grid-cols-2">
          {sections.map((section) => (
            <article key={section.title} className="rounded-[30px] border border-emerald-100 bg-white p-6 shadow-[0_16px_50px_rgba(16,24,40,0.06)]">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-600">Guia operacional</div>
              <h2 className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-emerald-950">{section.title}</h2>
              <div className="mt-5 grid gap-3">
                {section.items.map((item) => (
                  <div key={item} className="rounded-2xl border border-emerald-100 bg-emerald-50/60 px-4 py-4 text-sm leading-7 text-emerald-900">{item}</div>
                ))}
              </div>
            </article>
          ))}
